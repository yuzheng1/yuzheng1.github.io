...
abstract class Kohana_Controller_Rest extends Controller {
    ...
    /**
     * 返回结果是否有效 默认有效  OK 失败 NO
     * 默认为有效结果
     * @var unknown
     */
    protected $_resulState = self::OK;
    ...
    protected $_action_map = array
        ...
        HTTP_Request::OPTIONS => 'OPTIONS', //update by <huanglei@51syx.com>
    );
    ...
    protected function rest_output($data = array(), $code = 200) {
        ...
        $mime = File::mime_by_ext($this->output_format = 'json');

        $format_method = '_format_' . $this->output_format;
        // If the format method exists, call and return the output in that format
        if (method_exists($this, $format_method)) {
            $result = array(
                "state" => $this->_resulState,
            );
            if ($this->_resulState == self::OK) {
                $result['data'] = $data;
            } else {
                $result['eror'] = array(
                    "code" => $this->_resultErrorCode,
                    "errMsg" => $this->_resultErrorMsg,
                );
            }
            $output_data = $this->$format_method($result);
    ...
    private function _auth() {
        //无需执行 认证的action 或者认证方式
        if ((RestUser::AUTH_TYPE_OFF != $this->_auth_type) && !in_array($this->request->action(), $this->noNeedAuth)) {
            try {
                $this->_user = new RestUser($this->_auth_type, $this->_auth_source);
            } catch (Exception $e) {
                //认证失败处理
                $this->_resulState = self::NO;
                $this->_resultErrorCode = $e->getCode();
                $this->_resultErrorMsg = $e->getMessage();
                $this->rest_output(array("error")); //随便写，返回结果不为空就好了
